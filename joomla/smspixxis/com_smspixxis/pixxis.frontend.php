...
if( defined( 'PIXXIS_FRONTEND_PIXXIS_CONTROLLER_PHP' ) == true )
{
  return;
}
 ...
 */
define( 'PIXXIS_FRONTEND_PIXXIS_CONTROLLER_PHP', 1 );

/**
 *
 * Frontend Dispatcher for com_pixxis
 *
 *
* @package Pixxis
* @subpackage Frontend
 */
class PixxisFrontend
{
	/*
	 * task to proccess
	 * @var string
	 ...
	 */
	function PixxisFrontend( $task = 'default', $params = array( ) )
	{
		$this->task 		= $task;
		$this->params 		= $params;
		$this->html			= new PixxisFrontendHtml();
		$this->errorHandler = new PixxisError();

		if( isset( $params['PixxisUser'] ) )
		{
			$this->user = $params['PixxisUser'];
		}
		...
		if( $tok != -99 )
		{
			...
			{
				$this->errorHandler->Alert( JText::_( 'PIXXIS_INVALID_POSTRELOAD_TOKEN' ) );
			}
		}
	...
	function DoConfiguration()
	{
		...
		{							
			PixxisRedirect( $this->CreateRedirectUrl( 'configuration' ) );			
		}	
	...
	function DoSaveConfiguration()
	{
		//check if user press the cancel button
  		...
  		{
  			PixxisRedirect( $this->CreateRedirectUrl() );
  		}
  		...
  		if( !is_string( $number) )
  		{
  			$this->errorHandler->Alert( JText::_( 'PIXXIS_INVALID_PHONENUMBER' ) );
  		}
		...
		//create sql and query
  		$sql = "UPDATE /*com_pixxis->dosaveconfiguration*/ #__pixxis_joomlauser SET number='$number', comment='$comment' WHERE userid=". $this->user->joomlaID() ." AND id=". $this->user->PixxisID();
  		$this->db->setQuery( $sql );

	  	if( $this->db->query( $sql ) === false )
	  	{
	  		$this->errorHandler->Alert(  JText::_( 'PIXXIS_SQLQUERY_ERROR' ) , $this->db->getErrorMsg() );
  	  	}

  	  	PixxisRedirect( $this->CreateRedirectUrl() , JText::_( 'PIXXIS_CHANGES_SAVED' ) );
	}
	...
	function DoSmsarchiv()
	{
		if( $_SERVER['REQUEST_METHOD'] == 'POST' ) 
		{							
			PixxisRedirect( $this->CreateRedirectUrl( $this->task ) );			
		}	
		
		$mainframe = $this->params['mosMainframe'];				
		
		$limit    	= $mainframe->getUserStateFromRequest( "pixxis.frontend.smsarchiv.limit", "limit", 25, "int" );
	...
	{		
		...
		$limit    	= $mainframe->getUserStateFromRequest( "pixxis.frontend.phonebook.limit", "limit", 25, "int" );
	...
	function DoAddphonebookentry()
	{
		...
		$url .= 'task=phoneBook';
 		...
 		$number = JRequest::getVar( 'contactnumber', '' );
  		...
  		PixxisRedirect( $this->CreateRedirectUrl( 'phoneBook' ), JText::_( 'PIXXIS_PHONEBOOKENTRY_ENTRY_SUCCESSFULLY_ADDED' ) );
	}
	...
	{
		$option = JRequest::getVar( 'option', 'pixxis' );
		$ItemId = JRequest::getVar( 'ItemId', 0 );
		 ...
		 //get a complete comma seperated list form id's ( database )
  		...
  		if( empty( $ids ) )
  		{
  			PixxisRedirect( $this->CreateRedirectUrl( 'phoneBook' ) );
  		}
  		
  		 //something wrong
  		if( $id == -1 )
  		{
    		$this->errorHandler->Alert( JText::_( 'PIXXIS_INTERNAL_ERROR' )  );
    		die;
  		}
  		...
  		foreach( $ids as $k => $id )
  		{
  			...
  			{
      			$this->errorHandler->Alert( JText::_( 'PIXXIS_DEL_PHONEBOOK_ENTRY_FAILED' ) );
  			}	
  		
  		}
  		  	
  		PixxisRedirect( $this->CreateRedirectUrl( 'phoneBook' ) , JText::_( 'PIXXIS_DEL_PHONEBOOK_ENTRY_SUCCESSFULLY' ) );
	}
	...
	function DoDeleteusergroup()
	{
	  ...
	  //check what delete button is clicked
  		...
  		PixxisRedirect( $this->CreateRedirectUrl( 'usergroup' ) );
	}
	...
	function DoAddusergroup()
	{
 		...
 		if( isset($_REQUEST['back_button'] ) )
 		{
  		...
  		{
    		$this->errorHandler->Alert( JText::_( 'PIXXIS_ADD_GROUP_FAILED' ) );
  		}
	   ...
	   //nothing selected
   		if( count($userIDS) == 0 )
   		{
   			$this->errorHandler->Alert( JText::_( 'PIXXIS_ADD_GROUP_NO_SELECTION' ) );
   		}
		...
		{
      		$this->errorHandler->Alert( JText::_( 'PIXXIS_GROUPNAME_MISSING' ) );
   		}
	  ...
	  //now add every entry
  		foreach($userIDS as $userid)
  		{
    		...
    		{
      			$this->errorHandler->Alert( JText::_( 'PIXXIS_GROUP_ADD_MEMBER_FAILED' ) );
    		}
  		}
	...
	{					
		...
		{
			PixxisRedirect( $this->CreateRedirectUrl( 'phoneBook' ) , JText::_( 'PIXXIS_PHONEBOOKIMPORT_FAILED' ) );
		}
		...
		PixxisRedirect( $this->CreateRedirectUrl( 'phoneBook' ) , JText::_( 'PIXXIS_PHONEBOOKIMPORT_SUCCESSFULLY' ) );
	}
	...
	function DoSendSms( )
	{
		...
		$sql = " /*com_pixxis:dosendsms select global sms litmit */
				SELECT value FROM #__pixxis_config WHERE name='maxsms' LIMIT 1";
		...
		//check result and output a message
  		...
  		if( !is_null( $maxsms ) && $maxsms > 0 )
  		{
		  	...
		  	$sql = "SELECT COUNT(*) AS COUNTER FROM #__pixxis_sendsms";
  			$this->db->setQuery($sql);
  			...
  			if( $this->db->query($sql) == false )
  			{
  	 		...
  	 		{
  	 			$this->errorHandler->Alert( JText::_( 'PIXXIS_GLOBAL_LIMIT_REACHED' ) );
  	 		}
  		}
		...
		$ad       = JRequest::getVar('ad', '');
  		...
  		{ 
  			$this->errorHandler->Alert( JText::_( 'PIXXIS_SMS_BODY_EMPTY' ) );
  		}
		 ...
		 *  check sms sender
  		...
  		{
    		$this->errorHandler->Alert( JText::_( 'PIXXIS_INVALID_SENDER' ) );
  		}
	   ...
	   //check sms recv
  		...
  		{
      		$this->errorHandler->Alert( JText::_( 'PIXXIS_INVALID_RECIPIENT') );
      		die;
  		}
	  ...
	  //append advertisment to sms
 		...
 		if( strlen( $sms_body ) > 160 )
 		{
		  ...
		  //check provider here
  			if( $provider === false )
  			{
       			$this->errorHandler->Alert( JText::_( 'PIXXIS_NO_ACTIVE_PROVIDER' ) );
       			die;
  			}
  		...
  		foreach( $arr as $recv )
  		{
	       ...
	       if( strlen( $recv ) < 8 )
	       {
       		...
       		{
				$this->errorHandler->Alert( JText::_( 'PIXXIS_NOT_SUFFICIENT_FUNDS' ) );
       		}


       		{ //trigger plugins if available
       			$dispatcher = JDispatcher::getInstance();
       			JPluginHelper::importPlugin( 'pixxis' );
       			...
       			if( $abort == true )
       			{
       				PixxisRedirect( $this->CreateRedirectUrl() ,  JText::_( 'PIXXIS_PLUGIN_ABORT' ) );
       			}
       		}
       		
			/*
       		* Chamada da funcao alterada por PIXXIS para atender ao Gateway ByJG
    	   ...
    	   if( $ret == false )
    	   {
    	   		$errMsg .= JText::sprintf( 'PIXXIS_SEND_SMS_FAILED', $recv);
				$errMsg = $errMsg  . ' (' . $msg . ').';
       		}else{
            	...
            	{
                	$this->errorHandler->Alert( JText::_( 'PIXXIS_CHANGE_BALANCE_FAILED' ) );
            	}
    	       ...
    	       $provider->archiveSMS($text, $from, $to);   	          	               	    
            	...
            	$errMsg .= JText::sprintf( 'PIXXIS_SEND_SMS_SUCCESSFULLY',  $recv);
				$errMsg = $errMsg  . ' (' . $msg . ').';
       		}
  		}

  		PixxisRedirect( $this->CreateRedirectUrl(),  $errMsg );
	}
	...
	{		
		...
		if( isset( $_SESSION['prArray'][$token] ) ){
			  ...
			  //the token is not from com_pixxis !!! maybe a attack
			$this->errorHandler->Alert( JText::_( 'PIXXIS_POST_RELOAD_BLOCK' ) );
		}
