...
if( defined( 'PIXXIS_FRONTEND_PIXXIS_HTML_PHP' ) == true )
{
  return;
}
 ...
 */
define( 'PIXXIS_FRONTEND_PIXXIS_HTML_PHP', 1 );


/**
*  PixxisFrontendHtml is the html frontend class for com_pixxis
*
* @package Pixxis
* @subpackage Frontend
**/
class PixxisFrontendHtml
{
...
**/
  ...
  function SelectGroup( groupId )
  {
    ...
    var group = null;
    for( var idx = 0; idx < PixxisPhoneBookGroups.length; idx++ )
    {
        var object = PixxisPhoneBookGroups[idx];
    ...
    for( var idx = 0; idx < group.members.length; idx++ )
    {
		...
		for( var idx2 = 0; idx2 < PixxisPhoneBook.length; idx2++ )
		{

			var entry = PixxisPhoneBook[idx2];
...
?>
	...
	<div class="componentheading<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>">
     	<?php echo JText::_( 'PIXXIS_POLICY' ) ;
     	?>:
    </div>
...
?>
	<div class="componentheading<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>">
		<?php echo JText::_( 'PIXXIS_USERNAME' );?>: <?php echo $smsUser->_name; ?> (<?php echo JText::_( 'PIXXIS_BALANCE' );?>: <?php echo $smsUser->balance(); ?>)
	</div>
	...
	<div id="button_Wrap" style="overflow:hidden;padding:1px;">			
		<div id="btarchive" class="btns" style="float:left;width:100px;text-align:center;border:1px solid #ccc;padding:5px;background:#eee;">
			<form action="index.php" method="post">
				...
				<input style="" type="image" src="./media/com_pixxis/images/archive.png" height="32px" width="32px" name="smsarchiv_button" alt="<?php echo JText::_( 'PIXXIS_SMSARCHIVE' );?>" title="<?php echo JText::_( 'PIXXIS_SMSARCHIVE' );?>" />
				<div style="clear:both"></div>
				<span style=""><?php echo JText::_( 'PIXXIS_SMSARCHIVE' );?></span>
			</form>
		</div>
	</div>
...
<?php
	...
	{		
		echo "PixxisPhoneBook[$count] = new Object();";
		echo "PixxisPhoneBook[$count][\"id\"] = \"" . $val->id . "\";";
		echo "PixxisPhoneBook[$count][\"number\"] = \"" . $val->number . "\";";
		echo "PixxisPhoneBook[$count][\"name\"] = \"" . $val->name . "\";";
		echo "PixxisPhoneBook[$count][\"display\"] = \"" . $val->name . " ( " . $val->number . " ) \";";
...
function FilterPhoneBook()
{
	...
	if( val.length == 0 )
	{
		for( var idx = 0; idx < PixxisPhoneBook.length; ++idx )
		{
			var object = PixxisPhoneBook[idx];
			var option = document.createElement( 'option' );
...
<script type="text/javascript">
<!--
var PixxisPhoneBookGroups = new Array();
...
<?php
	...
	{		
		echo "PixxisPhoneBookGroups[$count] = new Array();";
		echo "PixxisPhoneBookGroups[$count][\"groupId\"] = \"" . $val->_id . "\";" . "\r\n";
		echo "PixxisPhoneBookGroups[$count][\"groupName\"] = \"" . $val->_name . "\";" . "\r\n";		
		echo "PixxisPhoneBookGroups[$count][\"members\"] = new Array();";
		
		$idx = 0;
		foreach( $val->_members as $member )
		{
			echo "PixxisPhoneBookGroups[$count][\"members\"][$idx] = " . $member->id  . ";" . "\r\n";
			$idx++;
		}
...
?>
		...
		?>
			...
			<div id="megawrap" style="width: 90%; margin: 0 auto;">
				<div id="leftwrap" style="width:350px;float:left;overflow:hidden;margin-right:5px;">
					<span id="senderlbl"><?php echo JText::_( 'PIXXIS_SENDER' );?></span>
					...
					<span id="dddlbl"><?php echo JText::_( 'PIXXIS_DDD' );?> (ex.: 85)</span>
					<input type="text" id="sms_ddd" name="sms_ddd" size="2" maxlength="2" value="85" style="border:1px solid #ccc" />
					<div style="clear:both"></div>
					<br />
					<span id="reciptlbl"><?php echo JText::_( 'PIXXIS_RECIPIENT' );?> (ex.: 88880000)</span>
					...
					<div id="lblmsg" class="sectiontableentry<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>">
						<span class="lbl_iput"><?php echo JText::_( 'PIXXIS_MESSAGE' );?> SMS: </span>
					...
					<?php if($adLen > 0){?>
						<span id="advertlbl"><?php echo JText::_( 'PIXXIS_ADVERTISMENT' );?>:</span>
			...
			<div style="clear:both"></div>
			<input type="submit" name="send_button" value="<?php echo JText::_( 'PIXXIS_SEND'); ?>" style="margin-top:5px;float:left;"/>
		</form>		
	</div>
	<div style="clear:both"></div>
	<br />
	<div class="adminform" style="text-align:right;">
    	<span>
			<small>SMS PIXXIS desenvolvido sobre: <a href="http://mysms.joomlacoder.de" target="_blank">MySMS</a></small>
		</span>
	</div> 
<?php
...
?>
	<form action="index.php" method="post">
		...
		<div class="componentheading<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>">
		   <?php echo JText::_( 'PIXXIS_MY_CONFIG' ); ?>: <?php echo $smsUser->userName(); ?>
		</div>
		
		<br/>
		
		<div id="inputconfigwrap" style="width:270px;">
			<span style="float: left;"><?php echo JText::_( 'PIXXIS_PHONENUMBER' ); ?>:</span>
			...
			<span style="float: left;"><?php echo JText::_( 'PIXXIS_COMMENT' );?>:</span>            
			...
			<input type="submit" name="change_button" value="<?php echo JText::_( 'PIXXIS_SAVE' ); ?>" />
			<input type="submit" name="cancel_button" value="<?php echo JText::_( 'PIXXIS_CANCEL' ); ?>" />			
		</div>       
...
?>
     <form action="index.php" method="get">
      ...
      <div class="componentheading<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>">
           <?php echo JText::_( 'PIXXIS_SMSARCHIVE' );?>: <?php echo $user->userName() ?>
       </div>
	   <div id="button_Wrap" style="overflow:hidden;padding:1px;">			
	<div id="button_Wrap" style="overflow:hidden;padding:1px;">			
		<div id="btarchive" class="btns" style="float:left;width:100px;text-align:center;border:1px solid #ccc;padding:5px;background:#eee;">
    ...
    <table border="0" class="adminform" width="100%" cellpadding="1" cellspacing="0">
	  <tr>
	    <td width="25%" class="sectiontableheader<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>"><b><?php echo JText::_( 'PIXXIS_DATE' ) ;?></b></td>
	    <td width="15%" class="sectiontableheader<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>"><b><?php echo JText::_( 'PIXXIS_RECIPIENT' );?></b></td>
	    <td width="60%" class="sectiontableheader<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>"><b><?php echo JText::_( 'PIXXIS_MESSAGE' );?></b></td>
	  </tr>
...
?>
	...
	<div class="componentheading<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>">
           <?php echo JText::_( 'PIXXIS_MY_PHONEBOOK' ) ;?>: <?php echo $user->userName() ?>
    </div>                
	...
	<div id="phbookEntries" style="overflow: hidden; padding:3px 0 10px;border-bottom:1px solid #ccc;">
		...
		<form id="phoneBookForm" action="index.php" method="get">
			...
			<input type="submit" value="<?php echo JText::_( 'PIXXIS_SEARCH' ); ?>"/>	   
			<input type="hidden" name="option" value="<?php echo $option; ?>" />
			<input type="hidden" name="ItemId" value="<?php echo $Itemid; ?>" />
			...
			<table border="0" class="adminform" width="100%" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td width="25%" class="sectiontableheader<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>"><b><?php echo JText::_( 'PIXXIS_NAME' ); ?></b></td>
					<td width="50%" class="sectiontableheader<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>"><b><?php echo JText::_( 'PIXXIS_PHONENUMBER' ); ?></b></td>
				</tr>
			...
			<div id="controlwrap" style="overflow: hidden;">
				<div id="cwLeft" style="float:left;">
					<input type="button" name="dPBB" id="dPBB" onClick="return DeletePhoneBookEntries();" value="<?php echo JText::_( 'PIXXIS_DELETE' );?>"/>
				</div>
...
function DeletePhoneBookEntries() {
	...
	<div id="phbooknewentries" style="overflow: hidden;padding:3px 0 10px;border-bottom:1px solid #ccc;">
		...
		<form action="index.php" method="post">
			...
			<div class="componentheading<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>">
				<?php echo JText::_( 'PIXXIS_NEW_ENTRY' ); ?>:
			</div>
                
			<div id="phentrieswrapper">
				<span><?php echo JText::_( 'PIXXIS_NAME' );?>:</span>
				<div id="inputname">
				   <input type="text" name="contactname" style="border: 1px solid #ccc" />
				</div>
				<span><?php echo JText::_( 'PIXXIS_PHONENUMBER' ); ?>:</span>
				<div id="inputnumber">
            ...
            <div id="phentriesbutton" style="margin-top:5px;">
				<input type="submit" value="<?php echo JText::_('PIXXIS_SAVE'); ?>"  />
            </div>
        </form>
	</div>
<br />
	...
	<div id="importphbook" style="overflow: hidden;padding:3px 0 10px;border-bottom:1px solid #ccc;">
		<form action="index.php" method="post" enctype="multipart/form-data">
			...
			<div class="componentheading<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>">
				<?php echo JText::_( 'PIXXIS_IMPORT_PHONEBOOK' ); ?>:
			</div>
                 
			<div id="importwrap">
				<span>
					<?php echo JText::_( 'PIXXIS_NAME' );?>
				</span>
				<div id="inputimport">
			...
			<div id="importbutton" style="margin-top:3px;">
				<input type="submit" name="save_button" value="<?php echo JText::_( 'PIXXIS_SAVE' )?>" />
            </div>
        </form>
	</div>
	...
	<div id="exportphbook" style="overflow: hidden;padding:3px 0 10px;border-bottom:1px solid #ccc;">
		<form action="index.php" method="post" enctype="multipart/form-data">
			...
			<div class="componentheading<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>">
				<?php echo JText::_( 'PIXXIS_EXPORT_PHONEBOOK' ); ?>:
			</div>
            
			<div id="exportbutton">
				<input type="submit" name="save_button" value="<?php echo JText::_( 'PIXXIS_EXPORT' );?>" />
			</div>
		</form>
	</div>
<?php
...
?>
	<div id="grouplist" style="overflow: hidden;padding:3px 0 10px;border-bottom:1px solid #ccc;">
		<form action="index.php" method="post">
			...
			<div class="componentheading<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>">
				<?php echo JText::_( 'PIXXIS_USER_GROUP' ) ;?>: <?php echo $user->userName(); ?>
			</div>
			...
			<table border="0" class="adminform" width="100%" cellpadding="0" cellspacing="0">
				<tr>
					<td width="25%" class="sectiontableheader<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>"><b><?php echo JText::_( 'PIXXIS_NAME' ); ?></b></td>
					<td width="60%" class="sectiontableheader<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>"><b><?php echo JText::_( 'PIXXIS_MEMBERS' );?></b></td>
					<td width="15%" class="sectiontableheader<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>"><b><?php echo JText::_( 'PIXXIS_ACTION' );?></b></td>
				</tr>
					...
					if( count($groupRows) > 0 ){
						foreach($groupRows as $entry ){
							...
							<tr>
								...
								<td  class="sectiontableentry<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>">
									<input type="submit" name="delete_button_<?php echo $entry->_id; ?>" value="<?php echo JText::_( 'PIXXIS_DELETE' ); ?>" />
								</td>
    ...
    <form action="index.php" method="post">
		...
		<div class="componentheading<?php echo $mosParameters->get( 'pageclass_sfx' ); ?>">
			<?php echo JText::_( 'PIXXIS_CREATE_NEW_GROUP' );?>:
		</div>
        
		<div id="groupnamewrap">
			<span><?php echo JText::_( 'PIXXIS_GROUPNAME' ); ?></span>
			<div id="inputgroupname">
				<input type="text" name="groupname" style="border:1px solid #ccc;"/>
			</div>
			
			<br />
			<span><?php echo JText::_( 'PIXXIS_MEMBERS' );?></span>
		...
		<div id="groupbutton">
			<input type="submit" name="save_button" value="<?php echo JText::_( 'PIXXIS_SAVE' ); ?>" />&nbsp;&nbsp;
			<input type="submit" name="back_button" value="<?php echo JText::_( 'PIXXIS_BACK' );?>" />
		</div>
	</form>
